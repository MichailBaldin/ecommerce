...
func TestConfig_NewWithDefaults(t *testing.T) {
	...
	assert.Equal(t, "products", cfg.ServiceName)
	...
	assert.Contains(t, cfg.PostgresURL, "dbname=products")
...
func TestConfig_NewWithEnvironmentVariables(t *testing.T) {
	...
	assert.Equal(t, "test-products", cfg.ServiceName)
...
func TestBuildPostgresURL_AllDefaults(t *testing.T) {
	...
	// Assert
	expected := "host=localhost port=5432 user=postgres password=password dbname=products sslmode=disable"
...
func setupTestEnvVars() {
	os.Setenv("PORT", "8080")
	os.Setenv("SERVICE_NAME", "test-products")
